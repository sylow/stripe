class StripeController < ApplicationController
  ...
  def webhook
    ...
    if user = User.where(stripe_customer_token: customer_token).first
      if notification_type == "invoice.payment_failed"        # if something goes wrong let our client know it
        UserMailer.payment_failure(user).deliver
        user.subscription.update_attribute(:active, false)
      elsif notification_type == "invoice.payment_succeeded"  # Activate this client if charge is successful
        user.subscription.update_attribute(:active, true)
      end
    end
